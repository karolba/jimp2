...
class Graph {
public:
    template <typename T> using VertexMap = unordered_map<Vertex*, T>;
...
void findAllChildren(const Graph &g, Vertex *v, vector<Vertex *> &allChildren) {
    const vector<Vertex *> &children = g.adjacencyList().at(v);
    for(Vertex *child : children) {
        allChildren.push_back(child);
        findAllChildren(g, child, allChildren);
    }
}
